...
WORD FrameScanLine;
  ...
  PIXEL DoubleFrame[ 2 ][ NES_DISP_WIDTH * NES_DISP_HEIGHT ];
  PIXEL *WorkFrame;
  WORD WorkFrameIdx;
#elif WORKFRAME_DEFINE == WORKFRAME_SINGLE
  PIXEL WorkFrame[ NES_DISP_WIDTH * NES_DISP_HEIGHT ];
#else
  PIXEL WorkLine[ NES_DISP_WIDTH ];
#endif
 ...
 */
  ...
  pPoint = WorkLine;
#else
  pPoint = &WorkFrame[ FrameScanLine * NES_DISP_WIDTH ];
#endif

  // Clear a scanline if screen is off
  if ( !( PPU_R1 & R1_SHOW_SCR ) )
  {
    InfoNES_MemorySet( pPoint, 0, NES_DISP_WIDTH * sizeof(PIXEL) );
  }
  else
  {
	  PIXEL	lpScanline[256+16];
	  PIXEL*	pScn = lpScanline+(8-loopy_shift);

	  WORD	ntbladr = 0x2000+(loopy_v&0x0FFF);
	  WORD	attradr = 0x23C0+(loopy_v&0x0C00)+((loopy_v&0x0380)>>4);
	  WORD	ntbl_x  = ntbladr&0x001F;
	  WORD	attrsft = (ntbladr&0x0040)>>4;
	  BYTE*	pNTBL = PPUBANK[ntbladr>>10];

#ifdef RAM_LACK
	  WORD	tileofs = (PPU_R0&R0_BG_ADDR)<<8;
	  WORD	tileadr;
	  BYTE	chr_h, chr_l;
#else
	  BYTE*	tileadr;
	  ...
	  for( WORD i = 0; i < 33; i++ )
	  {
		  ...
		  PIXEL*	pBGPAL = &PalTable[attr];

#ifdef RAM_LACK
		  tileadr = tileofs+pNTBL[ntbladr&0x03FF]*0x10+loopy_y;
		  chr_l = PPUBANK[tileadr>>10][ tileadr&0x03FF   ];
		  chr_h = PPUBANK[tileadr>>10][(tileadr&0x03FF)+8];
		  {
			  register WORD	c1 = ((chr_l>>1)&0x55)|(chr_h&0xAA);
			  register WORD	c2 = (chr_l&0x55)|((chr_h<<1)&0xAA);
			  pScn[0] = pBGPAL[(c1>>6)];
      ...
      MapperPPU( tileadr );
#else
		  tileadr = PPU_BG_Base+pNTBL[ntbladr&0x03FF]*0x40+loopy_y*8;
		  pScn[0] = pBGPAL[tileadr[0]];
		  pScn[1] = pBGPAL[tileadr[1]];
		  pScn[2] = pBGPAL[tileadr[2]];
		  pScn[3] = pBGPAL[tileadr[3]];
		  pScn[4] = pBGPAL[tileadr[4]];
		  pScn[5] = pBGPAL[tileadr[5]];
		  pScn[6] = pBGPAL[tileadr[6]];
		  pScn[7] = pBGPAL[tileadr[7]];
      // Character latch(For MMC2/MMC4)
      // Callback at PPU read/write
      MapperPPU( ( ( tileadr - ChrBuf ) >> 2 ) );
#endif /* RAM_LACK */
		  pScn+=8;
    ...
    {
	  for( WORD i = 0; i < NES_DISP_WIDTH; i++ ) {
		  pPoint[i] = PalTable[0];
	  }
    }
    else
    {
      InfoNES_MemoryCopy(pPoint, &lpScanline[8], NES_DISP_WIDTH * sizeof(PIXEL));
  ...
  if ( PPU_R1 & R1_SHOW_SP )
  {
    ...
    {
      InfoNES_MemorySet( pSprBuf, 0, 8 );
    }
    
    // Rendering sprite
    //pPoint -= ( NES_DISP_WIDTH - loopy_x );
